...
class Catalog{
	...
	public:
		...
		bool CreateTable(std::string tablename, Attribute attr, Index indices, int primary_key);
		...
		//输入：表格名称、属性名称、索引名称
		//输出：1-成功； 0-失败,包含异常
		int CreateIndex(std::string tablename, std::string tattr, std::string indexname);
