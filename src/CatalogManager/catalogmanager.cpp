...
//输出: 1-成功； 0-失败,包含异常 
// int CatalogManager::CreateTable(std::string table_name, Attribute attr, Index indices, int primary_key)
// {
// 	//检测是否有同名表的存在 
...
// 	int block_num = GetBlockAmount(TABLE_PATH);

// 	//遍历所有的块
// 	for (auto current_block = 0; current_block < block_num; current_block++) {
...
//输入：表格名称、属性名称、索引名称
//输出：1-成功； 0-失败,包含异常
// int CatalogManager::CreateIndex(std::string table_name, std::string tattr, std::string index_name)
// {
// 	if(!isTableExist(table_name))
// 	{
...
// 		return 0; 
// 	}
// 	if(isAttributeExist( table_name, tattr) == -1 )
// 	{
...
// 	//检查无误，正式开始添加索引
// 	cur_index.amount++;
// 	cur_index.attr_name[amount-1] = index_name;
// 	cur_index.whose[amount-1] = numberofattr;
	
// 	//由于原来的表已经计入，不能肯定它之后是否有其他信息，所以需要整个表删掉重新添加 
// 	if( DropTable(table_name) == 0 )
// 	{
// 		std::cout << "更新信息失败，无法删除原表，操作无效" << std::endl;
// 		return 0;		
// 	}
// 	if(CreateTable(table_name, cur_attr, cur_indexs, cur_attr.primary_key) == 0)
// 	{
...
int main(int argc, char const *argv[])
{
	...
	std::cout << cm.GetBlockAmount("./hhh.txt");
	return 0;
}
